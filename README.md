...
5. Set `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `ADMIN_EMAILS`, and `PUBLIC_BASE_URL` in `.env` or your hosting provider.
...
The public page only reads from Supabase. Admin panel writes are only allowed for signed-in admins whose email exists in Supabase Auth and either `admin_users` or `ADMIN_EMAILS`. Bookmarklet imports use the admin's short-lived Supabase session token.
...
2. Drag `Start New Week` and `Refresh Week` into the Chrome bookmarks bar. After signing in again later, refresh these bookmarks from the admin panel.
